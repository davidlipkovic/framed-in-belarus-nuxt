...
<script>
const collections = Array.from(document.querySelectorAll('.popUp'));

const body = document.querySelector('body');
const modalDiv = document.createElement('div');
modalDiv.classList.add('modal')
const modalDivContent = document.createElement('div');
modalDivContent.classList.add('content')
const btnClose = document.createElement('button');
btnClose.classList.add('close');
btnClose.innerText = 'X';
modalDiv.appendChild(modalDivContent);
modalDivContent.appendChild(btnClose);
body.appendChild(modalDiv);

for (let link of collections) {
  link.addEventListener('click', (event)=>{
    // if(event.currentTarget.data.alt){
    //   openDivModal(event.currentTarget.href, event.currentTarget.data.alt);
    // } else {
    //   openDivModal(event.currentTarget.href);
    // }
    openDivModal(event.currentTarget.href);
    event.preventDefault();
  });
}
function openDivModal(src, alt){
  let image = document.createElement('img');
  image.src = src
  if(alt){
    image.alt = alt;
  }
  modalDivContent.appendChild(image);
  modalDiv.classList.add('active');
  return closeDivModal();
}
function closeDivModal(){
  btnClose.addEventListener('click', ()=>{
    modalDiv.classList.remove('active');
    modalDivContent.remove('img');
  })
}
</script>
